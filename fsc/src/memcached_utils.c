...
void store_memcached(memcached_st *cache, const char *path, struct nipc_packet *packet) {
    ...
    memcached_return_t memcached_response = memcached_add(cache, key, strlen(key), packet->data, packet->data_length, (time_t)0, (uint32_t) 0);
    if (memcached_response == MEMCACHED_SUCCESS) {
        log_info(memcached_logger, "Se guardaron %d bytes para la clave %s", packet->data_length, key);
    } else {
        log_error(memcached_logger, "Error guardando datos en la clave %s: %s", key, memcached_strerror(cache, memcached_response));
    }
    free(key);
}
